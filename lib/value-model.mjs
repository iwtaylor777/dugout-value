...
export function estimateArbitrationSalary(
  ...
  // 20 percent. This is a guardrail, not a forced raise: a poor platform year
  // can leave a player flat or modestly lower, while a breakout is valued from
  // that season's production and arbitration class.
  const priorSalaryFloor = salaryMode === "arb1" ? 0 : priorSalary * 0.8;
  return Math.min(
    cap,
    Math.max(0.8 * inflation, priorSalaryFloor, classEstimate),
  );
}
